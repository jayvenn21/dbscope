...
pub async fn run_impact(
    ...
    let target = match analysis::ImpactTarget::parse(target_str) {
        Some(t) => t,
        None => {
            anyhow::bail!(
    ...
    let col_info = report
        ...
        .map(|c| format!(".{}", c))
        .unwrap_or_default();
    eprintln!("dbscope impact {}{}", report.target.qualified_table(), col_info);
    eprintln!();
    eprintln!("  Dropping or changing this will:");
    eprintln!("    - Affect {} table(s) (direct + transitive FK dependents)", report.fk_downstream_tables.len());
    ...
    eprintln!("    - Impact {}% of schema graph ({} of {} tables)", pct_graph, affected_tables, total_tables);
    eprintln!("    - Risk level: {} ({:.2})", risk_label, report.risk_delta);
    eprintln!();
    eprintln!("  Breakdown:");
    eprintln!("    FK reach (downstream):     {:.2}", report.risk_breakdown.fk_downstream_contrib);
    eprintln!("    Index coupling:           {:.2}", report.risk_breakdown.index_contrib);
    eprintln!("    Query usage weight:       {:.2}", report.risk_breakdown.queries_contrib);
    eprintln!();
    eprintln!("  FK upstream (this table depends on): {}", report.fk_upstream_tables.len());
    for t in &report.fk_upstream_tables {
        eprintln!("    - {}", t);
    }
    eprintln!("  Index dependencies on target: {}", report.index_dependencies.len());
