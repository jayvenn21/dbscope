//! Phase 4: CI mode — run schema/migration risk check and fail if over threshold or policy.
...
pub async fn run_ci(
    ...
    eprintln!("dbscope ci: {} tables, max table risk = {:.2}, overall = {:.2}, threshold = {:.2}",
        metrics.len(), max_risk, overall_risk, effective_threshold);

    let mut fail = false;
    if max_risk > effective_threshold {
        let over: Vec<_> = metrics.iter().filter(|m| risk_for(m) > effective_threshold).collect();
        eprintln!("FAIL: {} table(s) exceed risk threshold {:.2}:", over.len(), effective_threshold);
        for m in over {
    ...
    if pol.no_cycles {
        ...
        if !in_cycle.is_empty() {
            eprintln!("FAIL: {} table(s) in cycles (no_cycles: true):", in_cycle.len());
    ...
    if pol.no_orphans {
        ...
        if !orphans.is_empty() {
            eprintln!("FAIL: {} orphan table(s) (no_orphans: true):", orphans.len());
