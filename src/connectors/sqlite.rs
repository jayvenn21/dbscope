...
async fn fetch_views(pool: &SqlitePool) -> Result<Vec<TableMeta>, sqlx::Error> {
    let rows = sqlx::query_as::<_, (String,)>(
        r#"
        SELECT name FROM sqlite_master
...
async fn fetch_columns(
    ...
    for t in tables {
        ...
        for (ordinal_position, column_name, data_type, notnull, dflt, _pk) in rows {
...
async fn fetch_indexes_and_constraints(
    ...
    for t in tables {
        let list_sql = format!("PRAGMA index_list(\"{}\")", t.table_name);
        let list_rows = sqlx::query_as::<_, (i32, String, i32)>(&list_sql)
            .fetch_all(pool)
            .await?;
        ...
        for (_seq, index_name, unique) in list_rows {
            ...
            if is_auto {
                constraints.push(ConstraintMeta {
                    ...
                    constraint_type: if unique != 0 { "UNIQUE" } else { "PRIMARY KEY" }.to_string(),
                });
                continue;
            }

            let info_sql = format!("PRAGMA index_info(\"{}\")", index_name);
            let col_rows = sqlx::query_as::<_, (i32, i32, String)>(&info_sql)
                .fetch_all(pool)
                .await?;
        ...
        let pk_sql = format!("PRAGMA table_info(\"{}\")", t.table_name);
        let pragma_rows =
            sqlx::query_as::<_, (i32, String, String, i32, Option<String>, i32)>(&pk_sql)
                .fetch_all(pool)
                .await?;
        ...
        if !pk_cols.is_empty()
            && !constraints
                .iter()
                .any(|c| c.table_name == t.table_name && c.constraint_type == "PRIMARY KEY")
        {
            constraints.push(ConstraintMeta {
                schema_name: t.schema_name.clone(),
                table_name: t.table_name.clone(),
...
async fn fetch_foreign_keys(
    ...
    for t in tables {
        let fk_sql = format!("PRAGMA foreign_key_list(\"{}\")", t.table_name);
        ...
        type FkAccum = (String, String, Vec<String>, String, Vec<String>);
        let mut by_id: std::collections::HashMap<i32, FkAccum> = std::collections::HashMap::new();
