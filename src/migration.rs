//! Phase 4: Simulate schema changes from a migration file (DDL) for CI risk check.
        ...
        match stmt {
            Statement::Drop {
                ...
                for name in names {
                    ...
                    raw.tables.retain(|t| format!("{}.{}", t.schema_name, t.table_name) != q);
                    raw.columns.retain(|c| c.schema_name != schema || c.table_name != table);
                    raw.indexes.retain(|i| i.schema_name != schema || i.table_name != table);
                    raw.constraints.retain(|c| c.schema_name != schema || c.table_name != table);
            ...
            Statement::CreateTable { name, columns, .. } => {
                let (schema, table) = object_name_to_schema_table(name);
                let q = format!("{}.{}", schema, table);
                if !raw.tables.iter().any(|t| format!("{}.{}", t.schema_name, t.table_name) == q) {
