...
pub fn render<W: Write>(
    ...
    let medium = metrics
        .iter()
        .filter(|m| TableRisk::from_score(m.display_risk()) == TableRisk::Medium)
        .count();
    let low = metrics
        .iter()
        .filter(|m| TableRisk::from_score(m.display_risk()) == TableRisk::Low)
        .count();
    ...
    writeln!(
        w,
        "| Table | Centrality (in/out) | FK Depth (out/in) | Orphan | In cycle | Risk |"
    )?;
    writeln!(
        w,
        "|-------|---------------------|-------------------|--------|----------|------|"
